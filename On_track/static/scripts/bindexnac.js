var width = 1000,
    height = 350;
var bindexisvg = d3.select("#grafico").append("bindexisvg")
    .attr("height", height)
    .attr("width", width)
...
var bindexisvg = d3.select("bindexisvg");
...
bindexisvg.select(".legendOrdinal")
    .call(legendOrdinal)


bindexisvg.append("text")
    .attr("x", (width / 2))
    .attr("y", 50)
    ...
    .attr("font-weight", "bold")
    .style("fill", "white")
    .text("Tasa de accidentes viales por cada 1000,000 hab, México 2018");
...
d3.csv("/static/scripts/tasas2.csv", function(error, tasas) {
    ...
    var circles = bindexisvg.selectAll("d.Entidad")
        .data(tasas)
        .enter()
        .append("circle")
