...
function closeyear(y) {
  return refine("codigocierre")(y)
}

function inci(i) {
  return refine("incidente")(i)
}
    ...
    for (var i = 0; i < response.records.length; i++) {
      ...
      if (fields.delegacion_inicio === nombre) {
        ...
        markers.addLayer(L.marker([fields.geopoint[0], fields.geopoint[1]])
          .bindPopup(`<center><h4>AÑO: ${fields.ano}</br>MES: ${fields.mesdecierre}</br>HORA: ${fields.hora_creacion}</br>${fields.tipo_entrada}</br>INCIDENTE: ${fields.incidente_c4}</br></br>CODIGO: ${fields.codigo_cierre}</h4>
          ` + "<hd>" + `</center>`));
      } else if (fields.ano === nombre) {
        markers.addLayer(L.marker([fields.geopoint[0], fields.geopoint[1]])
          .bindPopup(`<center><h4>AÑO: ${fields.ano}</br>MES: ${fields.mesdecierre}</br>HORA: ${fields.hora_creacion}</br>${fields.tipo_entrada}</br>INCIDENTE: ${fields.incidente_c4}</br></br>CODIGO: ${fields.codigo_cierre}</h4>` + "<hd>" + `</center>`));
      } else if (fields.incidente_c4 === nombre) {
        markers.addLayer(L.marker([fields.geopoint[0], fields.geopoint[1]])
          .bindPopup(`<center><h4>AÑO: ${fields.ano}</br>MES: ${fields.mesdecierre}</br>HORA: ${fields.hora_creacion}</br>${fields.tipo_entrada}</br>INCIDENTE: ${fields.incidente_c4}</br></br>CODIGO: ${fields.codigo_cierre}</h4>` + "<hd>" + `</center>`));
      }
